...
case "$LLM_PROVIDER" in
  ...
  openrouter)
    ...
    if [[ "$LLM_MODEL" != openrouter/* ]]; then
      TARGET_MODEL="openrouter/$LLM_MODEL"
    fi
    API_BASE=""
    ;;
  custom|openai)
    API_KEY_VAR="${CUSTOM_API_KEY_VAR:-CUSTOM_API_KEY}"
    if [[ -z "${!API_KEY_VAR:-}" && -n "${OPENAI_API_KEY:-}" ]]; then
      API_KEY_VAR="OPENAI_API_KEY"
    fi
    TARGET_MODEL="$LLM_MODEL"
    if [[ "$LLM_MODEL" != openai/* ]]; then
      TARGET_MODEL="openai/$LLM_MODEL"
    fi
    API_BASE="${LLM_BASE_URL:-https://api.xah.io/v1}"
    ;;
  *)
    echo "ERROR: Unsupported LLM_PROVIDER='$LLM_PROVIDER'"
    echo "  Supported providers: nvidia, openrouter, custom, openai"
    exit 1
    ;;
esac
...
API_BASE_LINE=""
if [[ -n "$API_BASE" ]]; then
  API_BASE_LINE="      api_base: $API_BASE"
fi
